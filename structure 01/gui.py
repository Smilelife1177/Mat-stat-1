...
def create_gui(root):
    ...
    reset_btn = tk.Button(edit_frame, text="Скинути до початкових", state=tk.DISABLED)
    ...
    plot_btn = tk.Button(frame, text="Побудувати функції розподілу", state=tk.DISABLED)
    plot_btn.pack(fill=tk.X, pady=5)

    cdf_btn = tk.Button(frame, text=" ", state=tk.DISABLED)
    ...
    hist_canvas.get_tk_widget().pack(side=tk.RIGHT, fill=tk.BOTH, expand=True)
        ...
        # У функції create_gui після info_label
    confidence_label = tk.Label(frame, text="Рівень довіри (%):")
    confidence_label.pack()
    confidence_var = tk.DoubleVar(value=95.0)
    confidence_entry = tk.Entry(frame, textvariable=confidence_var)
    confidence_entry.pack()

    precision_label = tk.Label(frame, text="Точність (знаки після коми):")
    precision_label.pack()
    precision_var = tk.IntVar(value=4)
    precision_entry = tk.Entry(frame, textvariable=precision_var)
