...
def update_distribution_plot():
    ...
    # Helper function to generate simulated sample and estimate parameters
    def simulate_and_estimate(dist, params, size, dist_name):
        if dist == 'norm':
            simulated = norm.rvs(*params, size=size)
            est_params = (np.mean(simulated), np.std(simulated, ddof=1))
            return simulated, est_params, f"  Змодельована вибірка (Нормальний):\n    Оцінка середнього: {est_params[0]:.4f}\n    Оцінка стд. відхилення: {est_params[1]:.4f}\n"
        elif dist == 'expon':
            simulated = expon.rvs(scale=params[1], size=size)
            est_lambda = 1 / np.mean(simulated) if np.mean(simulated) != 0 else 0
            return simulated, (0, 1/est_lambda), f"  Змодельована вибірка (Експоненціальний):\n    Оцінка λ: {est_lambda:.4f}\n"
        elif dist == 'weibull_min':
            simulated = weibull_min.rvs(params[0], loc=params[1], scale=params[2], size=size)
            try:
                est_shape, est_loc, est_scale = weibull_min.fit(simulated, floc=0)
                return simulated, (est_shape, est_loc, est_scale), f"  Змодельована вибірка (Вейбулла):\n    Оцінка форми (k): {est_shape:.4f}\n    Оцінка масштабу (λ): {est_scale:.4f}\n"
            except:
                return simulated, None, f"  Змодельована вибірка (Вейбулла): Помилка оцінки параметрів\n"
        elif dist == 'uniform':
            simulated = uniform.rvs(loc=params[0], scale=params[1], size=size)
            est_loc, est_scale = np.min(simulated), np.max(simulated) - np.min(simulated)
            return simulated, (est_loc, est_scale), f"  Змодельована вибірка (Рівномірний):\n    Оцінка нижньої межі (a): {est_loc:.4f}\n    Оцінка масштабу (b-a): {est_scale:.4f}\n"
        elif dist == 'rayleigh':
            simulated = rayleigh.rvs(scale=params[1], size=size)
            est_sigma = np.sqrt(np.mean(simulated**2) / 2)
            return simulated, (0, est_sigma), f"  Змодельована вибірка (Релея):\n    Оцінка масштабу (σ): {est_sigma:.4f}\n"
        return None, None, ""
    ...
    if gui_objects['normal_var'].get():
        ...
        # Simulated sample
        simulated_data, est_params, sim_text = simulate_and_estimate('norm', (mean, std), n, 'Нормальний')
        if simulated_data is not None:
            results_text.append(sim_text)
            sim_hist, _, _ = gui_objects['ax_dist'].hist(simulated_data, bins=bin_count, color='pink', alpha=0.3, density=True, label='Змодельована вибірка (Нормальний)')
    ...
    if gui_objects['exponential_var'].get():
        if np.any(values < 0):
            ...
            if mean == 0:
                ...
                # Simulated sample
                simulated_data, est_params, sim_text = simulate_and_estimate('expon', (0, mean), n, 'Експоненціальний')
                if simulated_data is not None:
                    results_text.append(sim_text)
                    sim_hist, _, _ = gui_objects['ax_dist'].hist(simulated_data, bins=bin_count, color='lightblue', alpha=0.3, density=True, label='Змодельована вибірка (Експоненціальний)')
    ...
    if gui_objects['weibull_var'].get():
        if np.any(values < 0):
            ...
            try:
                ...
                # Simulated sample
                simulated_data, est_params, sim_text = simulate_and_estimate('weibull_min', (shape, loc, scale), n, 'Вейбулла')
                if simulated_data is not None:
                    results_text.append(sim_text)
                    sim_hist, _, _ = gui_objects['ax_dist'].hist(simulated_data, bins=bin_count, color='lightpurple', alpha=0.3, density=True, label='Змодельована вибірка (Вейбулла)')
    ...
    if gui_objects['uniform_var'].get():
        ...
        # Simulated sample
        simulated_data, est_params, sim_text = simulate_and_estimate('uniform', (loc, scale), n, 'Рівномірний')
        if simulated_data is not None:
            results_text.append(sim_text)
            sim_hist, _, _ = gui_objects['ax_dist'].hist(simulated_data, bins=bin_count, color='lightcyan', alpha=0.3, density=True, label='Змодельована вибірка (Рівномірний)')
    ...
    if gui_objects['rayleigh_var'].get():
        if np.any(values < 0):
            ...
            # Simulated sample
            simulated_data, est_params, sim_text = simulate_and_estimate('rayleigh', (0, sigma), n, 'Релея')
            if simulated_data is not None:
                results_text.append(sim_text)
                sim_hist, _, _ = gui_objects['ax_dist'].hist(simulated_data, bins=bin_count, color='lightyellow', alpha=0.3, density=True, label='Змодельована вибірка (Релея)')
