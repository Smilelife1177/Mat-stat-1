...
class Lab2Widget(QWidget):
    ...
    def initUI(self):
        ...
        labels = [
            "Кількість пар (n):", "М.спод. X:", "Ст.відх. X:",
            "М.спод. Y:", "Ст.відх. Y:", "Кореляція ρ:"
        ]
    ...
    def generate_data(self):
        try:
            ...
            # Генерація корелюючої двовимірної нормальної вибірки
            mean = [mux, muy]
    ...
    def calculate_statistics(self):
        ...
        # Перевірка нормальності (χ² Пірсона)
        chi2_stat, chi2_p = self.chi_square_test()
        normality_text = f"χ²-критерій: статистика = {chi2_stat:.4f}, p-value = {chi2_p:.4f}"
    ...
    def chi_square_test(self):
        """Простий χ²-критерій для перевірки нормальності X (одновимірний)"""
        if self.data_x is None:
            return 0, 1
        data = self.data_x
        # Інтервали: використовуємо 10 бінів
        hist, bin_edges = np.histogram(data, bins=10)
        n = len(data)
        expected = n / 10
        chi2 = np.sum((hist - expected)**2 / expected)
        from scipy.stats import chi2 as chi2_dist
        p_value = 1 - chi2_dist.cdf(chi2, df=9)  # 10-1 = 9 ступенів свободи
        return chi2, p_value
    ...
    def update_plots(self):
        ...
        if self.scatter_cb.isChecked():
            ...
            p = np.poly1d(z)
            ax.plot(self.data_x, p(self.data_x), "r--", linewidth=2)
            ax.set_title(f'Поле кореляції (r ≈ {np.corrcoef(self.data_x, self.data_y)[0,1]:.3f})')
            ax.set_xlabel('X')
            ax.set_ylabel('Y')
            ax.grid(True, alpha=0.3)
