...
def initialize_logic(objects):
    ...
    gui_objects['cdf_btn'].config(command=plot_exponential_distribution)  # Оновлено
...
def load_data():
    ...
    try:
        ...
        gui_objects['cdf_btn'].config(state=tk.NORMAL)  # Активуємо кнопку для експоненціального розподілу
...
def plot_distribution_functions():
    ...
    # Обчислюємо нормальний розподіл
    mean, std = np.mean(values), np.std(values)
    confidence = gui_objects['confidence_var'].get() / 100
    
    # Масштабуємо вісь X із запасом 10%
    x_min, x_max = np.min(values), np.max(values)
    x_range = x_max - x_min
    if x_range == 0:  # Обробка випадку, коли всі значення однакові
        x_range = 1
    x_margin = 0.1 * x_range  # 10% запасу
    x_lower = x_min - x_margin
    x_upper = x_max + x_margin
    
    # Генеруємо точки для нормального розподілу
    ...
    ax.set_xlim(x_lower, x_upper)  # Масштабована вісь X
    ax.set_ylim(-0.05, 1.05)  # Зберігаємо оригінальну вісь Y
...
def plot_exponential_distribution():
    ...
    # Перевіряємо, чи всі значення невід’ємні
    if np.any(values < 0):
    ...
    # Обчислюємо емпіричні ймовірності
    empirical_probs = np.arange(1, n + 1) / (n + 1)  # Метод i/(n+1)
    
    # Трансформуємо ймовірності для імовірнісної сітки
    ...
    if mean == 0:
        ...
        return
    lambda_param = 1 / mean  # λ = 1/середнє
    
    # Будуємо точки даних
    ax.scatter(sorted_values, y_values, color='green', label='Дані', s=50)
    
    # Будуємо теоретичну лінію
    x_theor = np.linspace(0, np.max(sorted_values) * 1.2, 100)  # Запас 20% для прямої
    ...
    y_max = np.max(y_values) * 1.2  # Запас 20% для осі Y
    ...
    ax.legend()
    ax.grid(True, linestyle='--', alpha=0.7)  # Увімкнена сітка
    ...
    # Тест Колмогорова-Смірнова для експоненціального розподілу
    confidence = gui_objects['confidence_var'].get() / 100
...
def remove_outliers():
    ...
    threshold = 3  # Поріг для Z-оцінки (можна зробити налаштовуваним)
    
    # Знаходимо потенційні аномалії
    ...
    # Формуємо список аномалій для показу користувачу
    outlier_info = [f"Індекс: {i}, Значення: {values[i]:.4f}, Z-оцінка: {z_scores[i]:.4f}" 
                    for i in outlier_indices]
    
    # Створюємо діалогове вікно для підтвердження
    dialog = tk.Toplevel()
    ...
    # Чекаємо закриття діалогу
    dialog.grab_set()
    dialog.wait_window()
    
    # Видаляємо обрані аномалії
    if selected_indices:
